...
## Importing
We import two datasets from the directory. `d_course` is a tiblle indicating the code, name and cluster[^2] of each course[^11]. `corpus` is a corpus containing the five most recent course catalogues of UCM. Course catalogues are published every year and contain a descritpion of each course that is one or two pages long.
...
```{r}
save(tf_idf, tf_idf_cluster, file = "results_tf_idf.RDATA")
```
...
checkboxGroupInput("course_selected", "Courses", sort(unique(tf_idf$`Course Title`)), unique(tf_idf$`Course Title`)[10 : 15], inline = TRUE)
...
checkboxGroupInput("cluster_selected", "Cluster", sort(unique(d_course$Cluster[!is.na(d_course$Cluster)])), unique(d_course$Cluster)[1 : 6], inline = TRUE)
...
```{r}
save(d_emergence, "results_emergence.RDATA")
```
...
checkboxGroupInput("course_selected_LDA", "Courses",
                   sort(unique(tf_idf$`Course Title`)),
                   c(unique(tf_idf$`Course Title`)[20 : 25],
                     "Computer Science", "Optimization", "Philosophy of Language"), inline = TRUE)
...
renderPlot({
  ...
  tidy(model, matrix = "gamma") %>%
    ...
    geom_col(show.legend = F) +
    facet_rep_wrap(~ topic, scales = "free", repeat.tick.labels = "bottom") +
    scale_x_reordered() +
    coord_flip() +
...
renderPlot({
  ...
  tidy(model, matrix = "gamma") %>%
    ...
    geom_col(show.legend = F) +
    facet_rep_wrap(~ facet, scales = "free", repeat.tick.labels = "bottom") +
    scale_x_reordered() +
    coord_flip() +
...
checkboxGroupInput("cluster_selected_LDA", "Cluster", sort(unique(d_course$Cluster[!is.na(d_course$Cluster)])), unique(d_course$Cluster[!is.na(d_course$Cluster)])[1:16])

radioButtons("n_topic_cluster", "Number of Topics",
             c("5"  = "LDA_5",
               "12" = "LDA_12",
               "17" = "LDA_17",
               "25" = "LDA_25"),
...
```{r visualization LDA label, message =  FALSE, echo = FALSE, fig.height=9}
