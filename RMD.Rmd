...
checkboxGroupInput("course_selected", "Courses", unique(tdm_course$`Course Title`)[1:10], "Conflict Resolution")
...
checkboxGroupInput("cluster_selected", "Cluster", unique(d_course$Cluster[!is.na(d_course$Cluster)]), "Sociology")
...
renderPlot({
  ...
  tidy(model, matrix = "beta") %>%
    left_join(tibble(topic_number = 1 : 100,
...
```{r visualization LDA label, message =  FALSE, eval = FALSE}
