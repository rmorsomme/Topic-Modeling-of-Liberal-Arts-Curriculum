...
```{r tidy, eval = FALSE}
d_description <- unnest_tokens(d_description, output = word, input = Description)
```
...
stem_hunspell <- function(term) {
  ...
  n_stem <- length(stems)
  stem   <- if(n_stem == 0) term       # if no stem in dictionary, return original term
            else stems[[n_stem]] # if multiple stems, return last one (most basic)
}
...
save(d_description, d_course, file = "data_prep.RDATA")
```
